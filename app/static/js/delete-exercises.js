...
// Get all the workouts depending on what page your on : getWorkoutsByTeam or getWorkoutsByAthlete - receive array of workouts
if (teamId) {
...
if (teamId) {
  const myTeamsWorkouts = await fetchWorkoutsByTeam(
    teamId,
    currentDate,
    coachId
  );
...
if (athleteId) {
  const myAthletesWorkouts = await fetchWorkoutsByTeam(
    teamId,
    currentDate,
    coachId
  );
