"use strict";
let myAthleteId = 1; // Should be ideally fetched from the sessionStorage
// let currentTeamId = 2;
let currentCoachId = 1; // FIXME: change to selectedDate later
let currentWorkout;
let currentBlocks;
...
currentCoachId = 2; // # FIXME: CHANGE TO DYNAMIC FROM PREVIOUS PAGE
...
async function initialData() {
  try {
    var athleteId = 1; // #TODO: SET THE ATHLETE ID FROM PREVIOUS PAGE
    //
...
$("#trainingTable tbody").on("click", "tr", function (event) {
  ...
  currentWorkout = clickedId.includes("team-id")
    ...
    : (function () {
        ...
        fetch(
          `getWorkoutsByAthleteDirect?coachId=${2}&athleteId=${athleteId}&date=${currentDate}`
        )
          .then((response) => response.json())
          .then((data) => {
